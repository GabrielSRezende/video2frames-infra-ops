...
Isso sobe:
- **LocalStack** (`video2frames-localstack`, porta `4566`) — cria a rede Docker `video2frames-net` (usada pelos `docker-compose.yml` de `video2frames-video-service`, `video2frames-processing-service` e `video2frames-notification-service` para se conectar a este LocalStack) e provisiona o bucket S3 `video2frames` + as 5 filas SQS.
- **Prometheus** (porta `9090`) — faz *scrape* de `/actuator/prometheus` dos 4 serviços via `host.docker.internal`, então funciona independente de como cada serviço está rodando (container ou host), desde que a porta esteja publicada.
- **Grafana** (porta `3000`, login `admin`/`admin`) — dashboard **"Video2Frames - Overview"** já provisionado.

> Suba este compose **antes** de subir o `docker-compose.yml` de `video2frames-video-service`/`video2frames-processing-service`/`video2frames-notification-service` — eles esperam a rede `video2frames-net` já existir.
...
O `web` é buildado com `ng build --configuration development`, que embute `http://localhost:8081/api` e `http://localhost:8082/api` como URLs de API — exatamente as portas publicadas pelos containers acima, então funciona sem nenhuma configuração extra. O CORS do `auth-service`/`video-service` já libera `http://localhost:4200` por padrão.
...
> Não rode o Modo 2 ao mesmo tempo que o `docker-compose.yml` individual de um serviço — ambos tentam publicar as mesmas portas de host (ex: `8082`, `5433`).
